...
"""
this view will return the home page of the  module.st
it renders the technique with module_number=1 from the model defaultapp.model
...
def homepage(request):
    ...
    return render(request,'psychoeducation/module_main_page.html',data)
...
def moduletechnique(request,technique_id):
    if request.POST.get('module_number') == None:
        try:
            ...
            return render(request,'psychoeducation/conversation_page.html',data)            
...
def show_list(request):
    ...
    if is_list_shown.status :
        return HttpResponseRedirect('/Psychoeducation about Depression and CBT/technique/'+str(technique.technique_id)+'/')
    list_to_show=ShowTechniqueBeliefsEventsNats.objects.filter(technique=technique)
    if not list_to_show.exists():
        return HttpResponseRedirect('/Psychoeducation about Depression and CBT/technique/'+str(technique.technique_id)+'/')
    return render(request,'psychoeducation/show_list.html',{'list_to_show':list_to_show ,'technique':technique})
